...
export class PreferenciasPage {
  ...
  obterPreferencias() {
    this.servidor.obterPreferencias().subscribe(
      sucess => {
        ...
        this.servidor
          .obterPreferenciasPorUsuario(this.usuario.usuario_final)
          .subscribe(interessePorUsuario => {
            ...
            for (let i = 0; i < interessePorUsuario.length; i++) {
              for (let x = 0; x < this.interesses.length; x++) {
                ...
                if (
                  interessePorUsuario[i].id_interesse ==
                  this.interesses[x].id_interesse
                ) {
  ...
  salvarPreferencias() {
    ...
    if (this.usuario.interesses.length == 0) {
      return this.toast
        .create({
          message: "É necessário ter ao menos um interesse marcado ",
          position: "botton",
          duration: 3000
        })
        .present();
